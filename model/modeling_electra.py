...
from model.modeling_bert import gelu, gelu_new, swish, BertEncoder, BertEmbeddings
...
def load_tf_weights_in_electra(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        name = name.replace('dense_1', 'classifier')
        name = name.split("/")
        # adam_v and adam_m are variables used in AdamWeightDecayOptimizer to calculated m and v
        # which are not required for using pretrained model
        if any(
                n in ["adam_v", "adam_m", "AdamWeightDecayOptimizer", "AdamWeightDecayOptimizer_1", "temperature",
                      "global_step"]
                for n in name
        ):
...
class ElectraForPreTraining(BertPreTrainedModel):
    ...
    def tie_weights(self):
        ...
        """
        self._tie_or_clone_weights(self.generator_predictions.decoder,
                                   self.electra.embeddings.word_embeddings)
        self._tie_or_clone_weights(self.generator.embeddings.word_embeddings,
                                   self.electra.embeddings.word_embeddings)
    ...
    def forward(
        ...
        sample_ids = temperature_sampling(g_logits, self.config.temperature, True)
        ...
        sequence_output, _ = electra_outputs[:2]
        d_logits, d_loss = self.discriminator_predictions(sequence_output, is_replaced_label)
        total_loss = self.config.gen_weight * g_loss + self.config.disc_weight * d_loss
        return (total_loss, g_loss, d_loss, d_logits, g_logits, is_replaced_label)
