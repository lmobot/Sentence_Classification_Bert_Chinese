...
def predict_Siamese(args, predict_dataset, model):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        if args.do_predict:
            ...
            result_list.extend(zip(indices, values))
        pbar(step)
    print(' ')
    if 'cuda' in str(args.device):
        torch.cuda.empty_cache()
...
def main(model_config_dict, do_operation='train', use_model='albert', siamese_flag=True):
    ...
    # bert Electra默认0.01 albert 0.0
    parser.add_argument("--weight_decay", default=0.0, type=float,
                        help="Weight deay if we apply some.")
    # bert默认0.05 albert 1e-6
    ...
    if args.do_train:
        ...
        if not args.siamese_flag:
            # predict general
            predict_dataset = ccf_load_and_cache_examples(args, tokenizer, data_type='dev')
            predict_result = predict(args, predict_dataset, model)

        if args.siamese_flag:
            # predict Siamese network
            predict_dataset = ccf_load_and_cache_examples_Siamese(args, tokenizer, data_type='dev')
            predict_result = predict_Siamese(args, predict_dataset, model)

        # for _label, _pred in predict_result:
        #     print(id2label[_label], _pred)
        '''
        result_path_999 = os.path.join(args.output_dir, args.model_name + '_999.csv')
        result_path_no999 = os.path.join(args.output_dir, args.model_name + '_no999.csv')
        logger.info("Save predict result to %s", result_path_999)
        unlabeled_data = get_dev_examples()
        result_list_999 = []
        result_list_no999 = []
        for _label, example in zip(predict_result, unlabeled_data):
            if _label[1] > 0.999:
                result_list_999.append(
                    {'id': example.guid, 'class_label': id2label[_label[0]], 'pred': round(_label[1], 4)})
            else:
                result_list_no999.append(
                    {'id': example.guid, 'class_label': id2label[_label[0]], 'pred': round(_label[1], 4)})

        df_s = pd.DataFrame(result_list_999, columns=['id', 'class_label', 'pred'])
        df_s.to_csv(result_path, columns=['id', 'class_label', 'pred'], index=False,
                    header=True,
                    encoding='utf-8')
        logger.info("Save unlabeled data done")
        # '''
        ...
        result_path = os.path.join(args.output_dir, args.model_name + '.csv')
        logger.info("Save predict result to %s", result_path)
        result_list = []
        for _id, _label in enumerate(predict_result):
            # 'content': example.text_a
            result_list.append(
                {'id': _id, 'class_label': id2label[_label[0]], 'rank_label': id2risk[_label[0]],
                 'pred': round(_label[1], 4)})

        df_s = pd.DataFrame(result_list, columns=['id', 'class_label', 'rank_label', 'pred'])
        df_s.to_csv(result_path, columns=['id', 'class_label', 'rank_label', 'pred'], index=False,
                    header=True,
                    encoding='utf-8')
        logger.info("Save unlabeled data done")
...
if __name__ == "__main__":
    # albert tiny predict model
    albert_base_config = './prev_trained_model/torch_albert_base/albert_config_base.json'
    albert_base_finetune_model = 'outputs/ccf_model/albert_base_zh_finetune_dataset3/albert_base_zh_epoch=3_batch=8_loss=0.1482_acc=0.9700_torch.bin'
    albert_base_finetune_config = {'config': albert_base_config, 'model': albert_base_finetune_model}

    # albert tiny predict model
    # albert_base_config = './prev_trained_model/torch_albert_base/albert_config_base.json'
    # albert_base_finetune_model = 'outputs/ccf_model/albert_base_zh_finetune/albert_base_zh_epoch=3_batch=8_loss=0.1482_acc=0.9700_torch.bin'
    # albert_base_finetune_config = {'config': albert_base_config, 'model': albert_base_finetune_model}

    # roberta predict model
    roberta_config = './prev_trained_model/torch_roberta_wwm_ext/roberta_wwm_ext_zh_config.json'
    roberta_finetune_model = 'outputs/ccf_model_dataset3/roberta_wwm_ext_zh_finetune/roberta_wwm_ext_zh_epoch=3_batch=8_loss=0.0854_acc=0.9720_torch.bin'
    roberta_finetune_config = {'config': roberta_config, 'model': roberta_finetune_model}

    # macbert base finetune
    macbert_config = './prev_trained_model/torch_macbert_base/macbert_config.json'
    macbert_finetune_model = 'outputs/ccf_model/macbert_base_finetune/macbert_base_epoch=2_batch=8_loss=0.2409_acc=0.9590_torch.bin'
    # macbert_finetune_model = 'outputs/ccf_model/macbert_base_finetune/macbert_base_epoch=4_batch=8_loss=0.1522_acc=0.9650_torch.bin'
    macbert_finetune_config = {'config': macbert_config, 'model': macbert_finetune_model}

    # electra finetune
    electra_config = './prev_trained_model/torch_electra_base/electra_base_discriminator_config.json'
    electra_finetune_model = './outputs/ccf_model/electra_base_discriminator_finetune_dataset3/electra_base_discriminator_epoch=3_batch=8_loss=0.1276_acc=0.9520_torch.bin'
    electra_finetune_config = {'config': electra_config, 'model': electra_finetune_model, 'learning_rate': 5e-5}

    # train dev test
    # albert predict
    # main(model_config_dict=albert_base_finetune_config, do_operation='test', use_model='albert', siamese_flag=False)
    # main(model_config_dict=macbert_finetune_config, do_operation='dev', use_model='bert', siamese_flag=False)
    # main(model_config_dict=roberta_finetune_config, do_operation='test', use_model='bert', siamese_flag=False)

    # roberta finetune
    main(model_config_dict=electra_finetune_config, do_operation='test', use_model='electra', siamese_flag=False)
